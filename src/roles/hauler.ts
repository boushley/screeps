...
export const hauler: RoleDefinition = {
  ...
  run(creep: Creep): void {
    const collecting = creep.memory.task !== "delivering";
    ...
    if (creep.memory.task === "delivering" || (!creep.memory.task && creep.store.getUsedCapacity() > 0)) {
      creep.memory.task = "delivering";
    ...
    // Collecting: pick up dropped energy first, then withdraw from containers
    creep.memory.task = "collecting";
